...
public class Updater {
    ...
    public void download(UpdaterConfig updaterConfig) {
        ...
        if (!UpdaterUtils.checkDownloadState(updaterConfig.getContext())) {
            //Toast.makeText(updaterConfig.getContext(),
        ...
        long downloadId = UpdaterUtils.getLocalDownloadId(
                updaterConfig.getContext());
        Logger.get().d("local download id is " + downloadId);
        if (downloadId != -1L) {
            FileDownloadManager fdm = FileDownloadManager.get();
            //获取下载状态
            int status = fdm.getDownloadStatus(updaterConfig.getContext(),
                    downloadId);
            switch (status) {
                //下载成功
                case DownloadManager.STATUS_SUCCESSFUL:
                    ...
                    Uri uri = fdm.getDownloadUri(updaterConfig.getContext(),
                            downloadId);
                    if (uri != null) {
                        //本地的版本大于当前程序的版本直接安装
                        if (UpdaterUtils.compare(updaterConfig.getContext(),
                                uri.getPath())) {
                            Logger.get().d("start install UI");
                            UpdaterUtils.startInstall(
                                    updaterConfig.getContext(), uri);
                            return;
                        }
                        else {
                            //从FileDownloadManager中移除这个任务
                            fdm.getDM(updaterConfig.getContext())
                               .remove(downloadId);
                        }
                    }
                    //重新下载
                ...
                case DownloadManager.STATUS_PENDING:
                    Logger.get().d("downloadId=" + downloadId + " ,status = STATUS_PENDING"); break;
