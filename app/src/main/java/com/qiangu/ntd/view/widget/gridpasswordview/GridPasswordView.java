package com.qiangu.ntd.view.widget.gridpasswordview;
...
import com.qiangu.ntd.base.utils.DensityUtil;
import com.qiangu.ntd.view.widget.gridpasswordview.imebugfixer.ImeDelBugFixedEditText;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTextColor = ta.getColorStateList(
                R.styleable.gridPasswordView_gpvTextColor);
        if (mTextColor == null) {
            mTextColor = ColorStateList.valueOf(getResources().getColor(
                    android.R.color.primary_text_light));
        }
        int textSize = ta.getDimensionPixelSize(
                R.styleable.gridPasswordView_gpvTextSize, -1);
        if (textSize != -1) {
            this.mTextSize = DensityUtil.px2sp(context, textSize);
        }

        mLineWidth = (int) ta.getDimension(
                R.styleable.gridPasswordView_gpvLineWidth,
                DensityUtil.dip2px(getContext(), 1));
        mLineColor = ta.getColor(R.styleable.gridPasswordView_gpvLineColor,
                DEFAULT_LINECOLOR);
        mGridColor = ta.getColor(R.styleable.gridPasswordView_gpvGridColor,
                DEFAULT_GRIDCOLOR);
        mLineDrawable = ta.getDrawable(
                R.styleable.gridPasswordView_gpvLineColor);
        if (mLineDrawable == null) {
            mLineDrawable = new ColorDrawable(mLineColor);
        }
        mOuterLineDrawable = generateBackgroundDrawable();

        mPasswordLength = ta.getInt(
                R.styleable.gridPasswordView_gpvPasswordLength,
                DEFAULT_PASSWORDLENGTH);
        mPasswordTransformation = ta.getString(
                R.styleable.gridPasswordView_gpvPasswordTransformation);
        if (TextUtils.isEmpty(mPasswordTransformation)) {
            mPasswordTransformation = DEFAULT_TRANSFORMATION;
        }

        mPasswordType = ta.getInt(R.styleable.gridPasswordView_gpvPasswordType,
                0);
    ...
    private void initViews(Context context) {
        ...
        mTransformationMethod = new CustomPasswordTransformationMethod(
                mPasswordTransformation);
    ...
    private void inflaterViews(Context context) {
        ...
        while (index < mPasswordLength) {
            ...
            LayoutParams dividerParams = new LayoutParams(mLineWidth,
    ...
    private void setCustomAttr(TextView view) {
        if (mTextColor != null) view.setTextColor(mTextColor);
    ...
    private ImeDelBugFixedEditText.OnDelKeyEventListener onDelKeyEventListener
            = new ImeDelBugFixedEditText.OnDelKeyEventListener() {
    ...
    @Deprecated private OnKeyListener onKeyListener = new OnKeyListener() {
    ...
    @Override public String getPassWord() {
        ...
        for (int i = 0; i < mPasswordArr.length; i++) {
            if (mPasswordArr[i] != null) sb.append(mPasswordArr[i]);
        }
        return sb.toString();
    }
     ...
     */
    @Override public void setPasswordVisibility(boolean visible) {
        for (TextView textView : mViewArr) {
            textView.setTransformationMethod(
                    visible ? null : mTransformationMethod);
    ...
    @Override public void setPasswordType(PasswordType passwordType) {
